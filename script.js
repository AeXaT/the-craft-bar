...
function renderFavDrawer() {
  ...
  list.innerHTML = favs.map(f => `
    <div class="fav-item">
      ...
      <button class="fav-remove" onclick="removeFav('${f.idDrink}')">✕</button>
    </div>`).join('');
}
...
function renderProfileFavourites() {
  ...
  const favs = getFavourites();
  if (!favs.length) {
    grid.innerHTML = `<div class="fav-empty-state">
      No favourites yet.<br>Browse <a href="./cocktails.html" style="color:#d4a043;">cocktails</a> and click the heart!
    </div>`;
    return;
  }
  grid.innerHTML = favs.map(f => `
    <div class="fav-card" >
      <div class="fav-card-img-wrap" onclick="openDetailModal('${f.idDrink}')">
        <img src="${f.strDrinkThumb || ''}" alt="${f.strDrink}" />
      </div>
      <div class="fav-card-body">
        <h4>${f.strDrink}</h4>
        <small>${f.strCategory || ''}</small>
        <button class="fav-card-remove" onclick="removeFav('${f.idDrink}')">✕ Remove</button>
      </div>
    </div>`).join('');
}
